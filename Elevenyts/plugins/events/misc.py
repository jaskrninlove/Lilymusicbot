...
async def auto_leave():
    """Auto-leave ONLY inactive groups after long inactivity."""
    while True:
        try:
            ...
            for ub in userbot.clients:
                ...
                try:
                    async for dialog in ub.get_dialogs():
                        ...
                        # DON'T leave if music active
                        if chat_id in db.active_calls:
                            continue

                        # NEW FIX:
                        # only leave dead groups after 7 days inactivity
                        ...
                        try:
                            ...
                            now = time.time()
                            msg_time = last_msg.date.timestamp()

                            inactive_days = (now - msg_time) / 86400

                            # Leave only if inactive 7+ days
                            if inactive_days >= 7:
                                await ub.leave_chat(chat_id)
                                left += 1

                                logger.info(
                                    f"Left inactive group: {dialog.chat.title}"
                                )
                    ...
                    logger.error(
                        ...
                        f"{ub.me.username if hasattr(ub, 'me') and ub.me else 'Unknown'}: {e}"
                    )

        except Exception as e:
            logger.error(f"Critical auto_leave error: {e}")
